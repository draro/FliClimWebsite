...
async function getBasicFlightStats(db: any, pilotId: string) {
  const logbooks = await db.collection('logbooks').find({
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const pilots = await db.collection('pilotprofiles')
      .find({ 
    ...
    const processedPilots = await Promise.all(pilots.map(async (pilot) => {
      const pilotData = {
        ...
        employers: pilot.employers?.filter(emp => emp.current) || [],
        createdAt: pilot.createdAt,
        certificateCount: pilot.publicProfile.showCertificates ? (pilot.certificates?.length || 0) : undefined
      };

      // Add flight experience if showFlightHours is enabled
      if (pilot.publicProfile.showFlightHours) {
        try {
          const flightStats = await getBasicFlightStats(db, pilot.userId.toString());
          pilotData.experience = flightStats;
        } catch (error) {
          console.error(`Failed to get flight stats for pilot ${pilot._id}:`, error);
